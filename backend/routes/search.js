...
router.get('/model-search', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    jsonLog({ type: 'search', query: `${brand} ${model}`, device, category, results: results.length, ms });
    ...
    jsonLog({ type: 'search_error', query: `${brand || ''} ${model || ''}`, error: err.message, ms: Date.now() - t0 });
    res.json({
